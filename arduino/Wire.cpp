...
I2C_HANDLE_T *TwoWire::i2cHandleMaster;
uint32_t TwoWire::i2cMasterHandleMEM[0x20];
...
uint8_t TwoWire::requestFrom(uint8_t address, uint8_t quantity, uint8_t sendStop)
{
	...
	int index = 0;
		...
		/* 7-bit address */
	// For7-bitaddressing,thefirstbyteofthereceivebuffermusthavetheslaveaddressin the most significant 7 bits and the least significant (R/W) bit = 1. Example: Slave Addr 0x53, first byte 0xA7
		rxBuffer[0] = (uint8_t) ((address << 1) | 1);
	...
	param.buffer_ptr_rec    = &rxBuffer[0];
	param.stop_flag         = sendStop;
...
uint8_t TwoWire::endTransmission(uint8_t sendStop)
{
  // transmit buffer (blocking)
	...
	/* Setup I2C paameters for number of bytes with stop - appears as follows on bus:
	   ...
	   value 1 - ack
	   value 2 - ack - stop */
	param.num_bytes_send    = txBufferLength;
	param.buffer_ptr_send   = &txBuffer[0];
